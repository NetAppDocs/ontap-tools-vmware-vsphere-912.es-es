...
. Extraiga la clave pública del certificado de firma de código emitido de Entrust (OTV_ISO_CERT.pem)
...
Si el sistema de almacenamiento se añade con un usuario de ámbito de SVM creado personalizado mediante el archivo json y desea actualizar a la versión 9,12, entonces
...
* Después de actualizar a la última versión de herramientas de ONTAP que tiene habilitada para FIPS, pero tiene una versión anterior de vCenter donde no es compatible con FIPS, la implementación seguirá funcionando.
